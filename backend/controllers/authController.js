  ...
  try {
    ...
    if (role === 'Patient') {
      ...
      if (patientRecords.length === 0) {
        return res.status(400).json({ message: 'Registration denied: You must be added to the hospital registry by an Admin before creating an account. Please verify your Phone Number.' });
      }
      if (!full_name) full_name = patientRecords[0].name;
      if (!email) email = patientRecords[0].email || `${phone}@patient.dkdhospital.com`;
    } else {
      if (!full_name || !email) {
